"""Тест стратегии бота для HYPEUSDT с настройками из config/settings_hype.yaml."""
...
def load_config(path: str = "config/settings_hype.yaml") -> dict:
...
def main():
    ...
    console.print("\n[bold cyan]📊 ТЕСТ СТРАТЕГИИ БОТА HYPEUSDT[/bold cyan]\n")
    ...
    console.print(f"[dim]Плечо:[/dim] {cfg['leverage']}x ⚠️  (1x isolated)")
    console.print(f"[dim]Конфиг:[/dim] config/settings_hype.yaml\n")
    ...
    try:
        ...
        console.print("[yellow]Попробую с 45 днями...[/yellow]")
        df = fetch_ohlcv(
            symbol,
            timeframe=cfg['timeframe'],
            days=45,
        ...
        console.print(f"[green]✓ Загружено {len(df)} свечей (45 дней)[/green]\n")
    ...
    if long_summary.n_liquidations > 0:
        ...
        recommendations.append("✓ Long: 0 ликвидаций - плечо 1x работает отлично")
    ...
    if short_summary.n_liquidations > 0:
        ...
        recommendations.append("✓ Short: 0 ликвидаций - плечо 1x работает отлично")
    ...
    if liq.level.value == "CRITICAL":
        ...
        recommendations.append("✓ Риск ликвидации SAFE - плечо 1x оптимально")
    ...
    if long_metrics['half_kelly'] > 0:
        ...
        recommendations.append("⚠️  Kelly отрицательный для Long - стратегия убыточна")
    ...
    if regime_stats.current_regime.value == "trending_up":
        recommendations.append("📈 Режим: Trending Up - текущие узкие параметры оптимальны")
    ...
    if long_summary.avg_entries < 1.1:
        recommendations.append("ℹ️  Long DCA редко срабатывает (avg_entries < 1.1) - узкая сетка работает")
    
    if short_summary.avg_entries < 1.1:
        recommendations.append("ℹ️  Short DCA редко срабатывает (avg_entries < 1.1) - узкая сетка работает")
    ...
    monthly_pnl_long = long_summary.total_pnl_pct / (cfg['history_days'] / 30)
    monthly_pnl_short = short_summary.total_pnl_pct / (cfg['history_days'] / 30)
    monthly_pnl_total = monthly_pnl_long + monthly_pnl_short
    
    console.print(f"[bold]Прогноз доходности (на $100 депозита):[/bold]")
    console.print(f"  Long:  ${monthly_pnl_long:.2f}/мес")
    console.print(f"  Short: ${monthly_pnl_short:.2f}/мес")
    console.print(f"  Total: ${monthly_pnl_total:.2f}/мес\n")
    
    console.print("[dim]Для детального анализа используйте:[/dim]")
    console.print("[dim]  python scripts/calc_volatility.py HYPEUSDT --config config/settings_hype.yaml[/dim]")
    console.print("[dim]  python scripts/backtest_long.py HYPEUSDT --config config/settings_hype.yaml[/dim]\n")
